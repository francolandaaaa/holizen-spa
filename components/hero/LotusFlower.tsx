...
// Layer config: inner (0) → outer (4)
// openRot at ~75% of full extension (3/4 open) — no lily pad intersection
// width increased ~35% for fuller, more lush petals
const LAYERS = [
  { count: 5,  radius: 0.060, length: 0.32, width: 0.148, closedRot: -0.22, openRot: 0.66, delay: 0.55, yOff: 0.030 },
  { count: 8,  radius: 0.115, length: 0.46, width: 0.202, closedRot: -0.16, openRot: 0.85, delay: 0.40, yOff: 0.015 },
  { count: 11, radius: 0.175, length: 0.60, width: 0.256, closedRot: -0.10, openRot: 1.04, delay: 0.25, yOff: 0.000 },
  { count: 13, radius: 0.235, length: 0.75, width: 0.304, closedRot: -0.05, openRot: 1.18, delay: 0.12, yOff: -0.015 },
  { count: 8,  radius: 0.295, length: 0.90, width: 0.352, closedRot:  0.00, openRot: 1.29, delay: 0.00, yOff: -0.030 },
] as const
...
export default function LotusFlower() {
  ...
  const fallingGeo      = useMemo(() => createPetalGeo(0.72, 0.30), [])
  ...
  useFrame((state) => {
    ...
    const raw   = Math.min((t - startRef.current) / BLOOM_DURATION, 1)
    const bloom = smoothstep(raw)
    bloomRef.current = bloom

    // Animate petal tilt per layer with cascading delay
    ...
    // Gentle float + scroll-driven Y rotation (only after bloom)
    if (groupRef.current) {
    ...
    // Falling petals on scroll (after bloom complete)
    if (bloom >= 1) {
      const scrollY    = typeof window !== 'undefined' ? window.scrollY : 0
      const scrollDiff = Math.abs(scrollY - prevScrollRef.current)
      prevScrollRef.current = scrollY

      if (scrollDiff > 3 && t - lastDropRef.current > 2.0 && fallingRef.current.length < 6) {
        lastDropRef.current = t
        const li  = 3 + Math.floor(Math.random() * 2)
        const cfg = LAYERS[li]
        const ang = Math.random() * Math.PI * 2
        fallingRef.current.push({
          id: _fid++, startTime: t, maxLife: 3.8 + Math.random() * 1.8,
      ...
      fallingRef.current.forEach((p, i) => {
        if (i >= 8) return
        p.x += p.vx * delta * 60; p.y += p.vy * delta * 60; p.z += p.vz * delta * 60
        p.vy -= 0.000065 * delta * 60
        p.rx += p.vrx * delta * 60; p.ry += p.vry * delta * 60; p.rz += p.vrz * delta * 60
        ...
        mesh.visible = true
        mat.opacity = Math.max(0, 1 - Math.max(0, (t - p.startTime - p.maxLife * 0.65) / (p.maxLife * 0.35)))
      })
  ...
  return (
    <group ref={groupRef} scale={1.65}>
      ...
      {/* Lily pad — positioned clearly BELOW the flower (y=-0.28 local)
          so it never intersects the open petals. The outermost petal tips
          at 3/4 open sit at y≈+0.22 local, well above the pad. */}
      <mesh rotation={[-Math.PI / 2, 0.6, 0]} position={[0, -0.28, 0]}>
        <circleGeometry args={[1.10, 48]} />
